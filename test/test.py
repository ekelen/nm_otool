...
class Base(TestCase):
	...
	def compare_otool(self, test_files):
		for f in test_files:
	...
	def compare_flag_all(self, test_files):
		for f in test_files:
			with self.subTest(f=f):
				try:
					ftnm = subprocess.check_output([nm_path, "-a", os.path.join(self.test_path, f)], stderr=subprocess.DEVNULL)
					nxnm = subprocess.check_output(["nm", "-a", os.path.join(self.test_path, f)], stderr=subprocess.DEVNULL)
					self.assertEqual(ftnm.decode(), nxnm.decode(), msg=f'{f} does not match.')
				except subprocess.CalledProcessError:
					ftnm = subprocess.check_output([nm_path, "-a", os.path.join(self.test_path, f)], stderr=subprocess.DEVNULL)
					self.assertFalse(ftnm, msg=f'{f} not causing error.')
...
class Easy(Base):
	...
	def test_easy_flag_all(self):
		""" All the easy ones."""
		self.compare_flag_all(["test_facile", "test_moins_facile", "test_half_obj", "test_wrong_lc_command_size"])

	# def test_easy_flag_no_undef(self):
	# 	""" All the easy ones."""
	# 	self.compare(["test_facile", "test_moins_facile", "test_half_obj", "test_wrong_lc_command_size"], flags=["-u"])

class Easyot(Base):
	def test_easy(self):
		""" All the easy ones."""
		self.compare_otool(["test_facile", "test_moins_facile", "test_half_obj", "test_wrong_lc_command_size"])
...
class T32ot(Base):
	...
	def test_ls_32(self):
		self.compare_otool(["MachO-OSX-x86-ls"])

	def test_32_exe_hard(self):
		self.compare_otool(["32_exe_hard"])

class T32ot_Hard(T32ot):
	def test_Helloworld_32(self):
		self.compare_otool(["MachO-iOS-armv7s-Helloworld"])

	def test_32_openssl(self):
		self.compare_otool(["MachO-OSX-ppc-openssl-1.0.1h"])

	def test_32_arm117(self):
		self.compare_otool(["MachO-iOS-arm1176JZFS-bash"])
...
class Fatot(Base):
	...
	def test_fat_32_ppc_i386(self):
		self.compare_otool(["MachO-OSX-ppc-and-i386-bash"])

	# def test_all_fat_all(self):
	# 	""" All the fat ones."""
	# 	files = os.listdir(self.test_path)
	# 	self.compare_flag_all(files)
...
class Dylib(Base):
	...
	# def test_all_fat_flag_all(self):
	# 	""" All the fat libs ones."""
	# 	files = os.listdir(self.test_path)
	# 	self.compare_flag_all(files)
